...
import { suggestedList } from '@app/shared/data/suggestedList';
...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                case url.endsWith('/products/suggestedSearchList') && method === 'POST':
                    return getSuggestedSearchList();
        ...
        function getSuggestedSearchList() {
            ...
            var suggestedResults = suggestedList.filter(option => option.toLowerCase().includes(body.toLowerCase()))
            return ok(suggestedResults)
        }
        function updateUser() {
            if (!isLoggedIn()) return unauthorized();
